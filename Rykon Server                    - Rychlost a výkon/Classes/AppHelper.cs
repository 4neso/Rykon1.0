...
namespace RykonServer
{
 public static    class Extended{
    ...
    public class AppHelper
    {
     ...
     internal static void ViewHowToRunAsAdmin(bool UseCusForm=false)
        {
            try
            {

                string f = System.IO.Path.GetTempPath() + "HowToRunAsAdminPic.png";
                System.IO.File.WriteAllBytes(f, HowToRunAsAdminBytes);

                if (UseCusForm)
                {
                    Forms.FrmMessagBox Msg_frm = new Forms.FrmMessagBox("Admin Right required","Please run as adminstrator \r\n as seen on the pic ",f);
                    Msg_frm.ShowDialog();
                }
                else
                {
        ...
        internal static string LastPeice(string Text, string Deli)
        {
        ...
        internal static int BoolTrueUnkown(string val)
        {
            // yes 1 true right

            val = val.Trim().ToLower();
            if (val == "true" || val == "1" || val == "right")
                return 1;
            else   if (val == "false" || val == "0" || val == "wrong")
                    return 0;
                else  return -1;
            // no 0 false wrong
        }
        ...
        internal static void EnormusMessageBox(string MessageBoxCap)
        {
